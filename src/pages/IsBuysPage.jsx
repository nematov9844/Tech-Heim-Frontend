...
export default function IsBuysPage() {
    ...
    useEffect(() => {
        async function fetchData() {
            try {
                ...
                setLaptopData(data1.filter(Item=>Item.isBuy));
                setCameraData(data2.filter(Item=>Item.isBuy));
                setAccessoriesData(data3.filter(Item=>Item.isBuy));
                setSmartPhoneData(data4.filter(Item=>Item.isBuy));
                setGamingData(data5.filter(Item=>Item.isBuy));
            } catch (error) {
